    ...
    <div class="table-responsive">
      <table class="table card-table">
        ...
        <div class="d-flex justify-content-start mb-3 px-3">
            @if ($canManage)
                <a href="{{ route('ekskul.create') }}" class="me-2">
                    <button type="button" class="btn btn-primary" title="Tambah Kegiatan"><i class='bx bxs-user-plus'></i></button>
                </a>
            @endif
        ...
        <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->foto)
                            <a href="{{ Storage::url($item->foto) }}" target="_blank">
                                <img src="{{ Storage::url($item->foto) }}" alt="Foto Kegiatan" class="img-fluid rounded" width="80">
                            </a>
                        @else
                    ...
                    <td>
                        ...
                        @if ($status == 'Pending' && $canApprove)
                            <form action="{{ route('ekskul/update/status.updateStts', $item->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('PUT')
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    @if ($isAdmin || $isWaka || $item->user_id == $user->id)
                                        ...
                                        <form action="{{ route('ekskul.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                            @csrf
                                            @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="{{ $isKepalaSekolah ? 6 : 7 }}" class="text-center">Tidak ada data yang ditemukan.</td>
                </tr>
            @endforelse
        </tbody>
      </table>
