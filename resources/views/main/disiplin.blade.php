            ...
            <div class="table-responsive">
              ...
              @endif --}}
                        ...
                        <form action="{{ route('disiplin.index') }}" method="GET" class="d-flex">
                            <input class="form-control me-2" type="text" name="search" placeholder="Cari nama & masalah"
                                value="{{ request()->input('search') }}">
                            <input class="form-control me-2" type="month" name="month"
                                value="{{ request()->input('month') }}">
                            <button type="submit" class="btn btn-primary me-2">Cari</button>
                            <a href="{{ route('disiplin.export.pdf', ['month' => request('month')]) }}"
                                class="btn btn-danger me-2" title="Export PDF"><i class='bx bxs-file-pdf'></i></a>
                            <a href="{{ route('disiplin.export.excel') }}" class="btn btn-success" title="Export Excel"><i
                    ...
                    <div class="d-flex justify-content-start mb-3 px-3">
                        @if ($canManage)
                            <a href="{{ route('disiplin.create') }}" class="me-2">
                                <button type="button" class="btn btn-primary" title="Tambah Data"><i
                    ...
                    <tbody>
                        @forelse ($disiplin as $dicipline)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($dicipline->tanggal)->locale('id')->translatedFormat('l, d F Y') }}
                                </td>
                                <td>
                                    @if ($dicipline->foto)
                                        <a href="{{ Storage::url($dicipline->foto) }}" target="_blank">
                                            <img src="{{ Storage::url($dicipline->foto) }}" alt="Foto Disiplin"
                                ...
                                <td>
                                    ...
                                    @if ($isPending && $canApprove)
                                        <form action="{{ route('disiplin/update/status.updateStts', $dicipline->id) }}"
                                            ...
                                            <div class="dropdown-menu">
                                                @if ($canManage && !$isApproved)
                                                    <a class="dropdown-item"
                                                        href="{{ route('disiplin.edit', $dicipline->id) }}">
                                                        <i class="bx bx-edit-alt me-1"></i> Edit
                                                    </a>
                                                @endif

                                                @if ($isAdmin || $isWaka || ($isStaff && $dicipline->user_id == $user->id))
                                                    <form action="{{ route('disiplin.destroy', $dicipline->id) }}"
                                                        method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $isKepalaSekolah ? 7 : 8 }}" class="text-center">Tidak ada data yang
                                    ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
